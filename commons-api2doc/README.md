...
1.  文档框架，右侧内容改为自适应布局。（DONE）
2.  文档框架，在 URL 中加 tag 来标识每个页面。（DONE）
3.  文档框架，左侧菜单是悬浮模式，方便用户截图。（DONE）
4.  文档页面，“URL示例”改为“请求示例”，并支持所有的 METHOD 。
5.  文档页面，“请求参数”表格，加上“参数位置”这一列。
6.  文档页面，加上“返回示例”这一项，用 JSON 串表示。
7.  文档页面，对没有指定的 METHOD 需要显示为所有方法。（DONE）
8.  后台服务，提供 API 元数据接口，以支持 api2test 项目的研发。（DONE）
