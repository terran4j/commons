...
import com.terran4j.commons.rediscow.dsyn.DSynchArgs;
...
public class DSynchronizedTest extends BaseCacheTest {
	...
	public void testDSynchronizedMultiThread() throws Exception {
			...
			threads[i] = new Thread(new Runnable() {
				...
				public void run() {
					for (int j = 0; j < exeCount; j++) {
						try {
							dSynchronizedService.incrementAndGet(key, -1, -1);
						} catch (Throwable e) {
							failed.add(e);
						}
					}
		...
		Assert.assertEquals(threadCount * exeCount, dSynchronizedService.getValue(key));
	}
}
